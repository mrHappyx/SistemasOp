...
public class Loteria {
    ...
    private Proceso seleccionarProceso() {
        ...
        for (Proceso p : procesos) {
            if (p.estado == 1) {
                for (int i = 0; i < p.boletos; i++) { // Utiliza el número de boletos en lugar de la prioridad
                    listaBoletos.add(p);
                }
            }
        }
    ...
    private void imprimirReporte() {
        ...
        for (Proceso p : procesos) {
            System.out.println("Proceso " + p.id + " - Tiempo Restante: " + p.tiempoRestante + " - Estado: " + (p.estado == 1 ? "Listo" : "Bloqueado") + " - Prioridad Original: " + p.prioridadOriginal + " - Usuario: " + p.usuario + " - Boletos: " + p.boletos);
        }
    }
}